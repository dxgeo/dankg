...
[`render::dot`](dot.md) renders the same distinction with `fillcolor`.
`tui/draw.rs` renders it with a different border glyph. Each renderer has
its own way of rendering the same underlying `NodeKind::Block` fact.
