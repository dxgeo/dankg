...
| 30 | Manifest schema v2 | Adds a =blocks= array (name, =line=, =end_line=) per file, straight off the same =BlockRef= fields =result.rs= already tracks | A richer glue script needs block-level source position to compare against, never a paragraph's own text; DanKG hands over structure, judgment stays glue's (decision 27). |
