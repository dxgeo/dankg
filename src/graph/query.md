...
`links_for` is what the TUI's own cross-reference panel reads: a node's
outgoing/incoming links and produced/read relations, the "surprising
cross-link" visibility a plain containment tree does not otherwise carry.
`Graph::incoming_link_count` only counts; nothing before this listed the
actual edges. `Contains` is deliberately excluded -- that structure is
the tree's own job (a node's `parent`), not a cross-reference this
function owes a caller a second time.

```rust name=links_for path=graph/query.rs
