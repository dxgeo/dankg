...
```rust name=module_doc path=md/inline.rs
...
```rust name=run_and_text path=md/inline.rs
impl<'a> Parser<'a> {
    fn run(&mut self) {
...
```rust name=line_break_and_brackets path=md/inline.rs
impl<'a> Parser<'a> {
...
```rust name=to_inlines path=md/inline.rs
...
```rust name=tests path=md/inline.rs
#[cfg(test)]
mod tests {
    use super::*;
