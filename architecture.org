...
Milestone 8. Literate programming put the prose and the code that implements it
...
milestone 8 adds is a database as the thing being written to, and the relations
...
refused. =[db.*]= is parsed today even though milestone 8 implements it, so a
...
6. [DONE] =render/html.rs= + =render/assets.rs= and the JS interaction layer.
   ...
   goldens.
7. =eval/= plan, run, write-back, =dankg check=.
8. Literate database management, over DuckDB. Depends on 7: it is the same
   plan-run-write-back machinery pointed at a database instead of a process.
9. =dankg serve= -- deferred, opt-in, only if the static path proves insufficient.
