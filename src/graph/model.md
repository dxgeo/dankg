...
The graph itself: `Node`, `Edge`, `NodeId`, and the several kinds each of
the first two come in. Edges are stored directed. `reciprocated` is
computed only once the whole corpus is known. When `a -> b` and `b -> a`
both exist, both get marked. Every renderer renders one undirected edge
