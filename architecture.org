...
| 21 | Eval "all" scope   | =--all= = DAG leaves only; =--each= = every block | A pure dependency needs no redundant standalone spawn just for being named. |
| 22 | Block name scope   | Whole file, flat; =deps= resolves the same way, any heading to any other | Heading-scoped uniqueness (tried, reverted) broke a plain sequence of sibling sections each depending on the last. |
| 23 | Tangle block scope | Exactly decision 20's node scope (named, top-level) | A block invisible to the graph and un-eval-able alone is a blind spot in the shipped program. |
| 24 | Tangle placement   | Heading containment + document order; =deps= not consulted | Placement is presentational structure; =deps= stays eval's own DAG concern. |
| 25 | Command name       | =dankg tangle=, not =compile=               | DanKG assembles; a configured command does any actual compiling, and some languages compile nothing at all. |
| 26 | Tangle corpus scope | One file unchanged; a directory (or several paths) walks the corpus, nesting each contributing file under its own subdirectory once more than one is involved | A crate is a whole-corpus artifact; =deps== has no bearing since tangle never reads it. |
| 27 | Module glue        | An external, per-language, spawned =[tangle.<lang>] glue= command; never DanKG's own code | Matches the spawned-command pattern (decision 1); lets an extension do things DanKG's own code must not (read a block's source). |
| 28 | Per-file tangle hints | =dankg.tangle.public= frontmatter, surfaced to =glue= only through a sidecar manifest | Visibility is a per-file authorial call; whether the mechanism runs at all stays root-config only (decision 27). |
| 29 | Cross-file =deps=  | =deps=other.md#name= reaches another file's top-level block; =--block=/=--all=/=--each=' own targets stay one file (decision 19 unchanged) | Concatenation (decision 11) makes "the chain crosses a file" and "the chain crosses a language" the same *kind* of non-problem; only resolution needed widening, not the execution model. |
...
documentation -- so it costs no new vocabulary once =project.org='s own
...
=.md= path, and =public= flag. Hand-rolled JSON, the same reasoning as
=render/json.rs=: DanKG takes no crates, and the shape here is small and
fully under our control -- small enough that the escaper is literally
shared (=render::json::string=) rather than reimplemented a second time
for one more, equally small, JSON writer.
...
- The manifest schema (decision 28) is =version: 1= and has exactly the
  three fields a Rust =glue= script needs today. Whether a richer glue
  implementation ever needs more (declaration order across a directory,
  say, rather than whatever order tangle happened to write files in) is
  unanswered until one is actually written.
