...
`dankg eval`'s five pieces. Nothing under this module is ever reachable
from `dankg graph` -- evaluation only ever runs when this command is
invoked directly, never as a side effect of drawing or checking a corpus
