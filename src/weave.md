...
A label has to be an anchor both backends can carry. It holds
letters, digits, `-` and `_` and nothing else. That is the same rule
`slugify` already applies to a heading, which is what keeps a figure
label and a heading slug interchangeable as a fragment. A label
failing it warns at its own line and is dropped, the same as a
collision.
...
`unclosed label`, pointing into generated `.typ` -- exactly the error
decision 64 exists to keep an author from ever seeing. `.` and `:`
