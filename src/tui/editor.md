...
```rust name=handoff path=tui/editor.rs
...
`app.rs` needs to know, ahead of calling `open`, whether the editor is
about to take over the whole terminal or land quietly in its own
pane, since that is what decides whether raw mode gets suspended at
all -- this module still owns none of that decision itself, per the
header above. `open` makes the identical check for its own, narrower
reason: which argv to spawn. One function, `pane_available`, is the
single place that check lives, so the two call sites can never drift
apart.

```rust name=pane_available path=tui/editor.rs
...
Wrapping the resolved argv in a tmux split, rather than resolving a
second one, is what lets a tmux reader and a plain-terminal reader
share every test above -- `tmux_argv` only ever sees what `resolve`
already decided.

```rust name=split_flags path=tui/editor.rs
...
```rust name=tmux_argv path=tui/editor.rs
...
Persisting which pane to reuse cannot live in `App`'s own memory --
quitting and reopening `dankg tui` starts a fresh process with no
memory of the last one, even though the tmux pane from that last
session might still be sitting right there. tmux itself is the
persistence layer instead: a window-scoped user option, set once
`open` creates a pane and read back by the next `open` call in the
same tmux window, however many `dankg tui` processes have come and
gone in between. It disappears on its own once the window itself
closes -- the same "nothing left to go stale" property a durable
file would not have had.

```rust name=tracked_pane path=tui/editor.rs
...
`tmux split-window` already makes a freshly spawned pane the active
one on its own, with no help needed from `open`. Reuse has no such
help: `tmux send-keys` only types into `pane`, it does not switch the
client's attention there, so without an explicit nudge the reader
would stay looking at the tree with no visible sign `enter` did
anything.

```rust name=select_pane path=tui/editor.rs
...
Reuse also needs to know whether `tracked_pane` is not just still
there, but still running the editor it was handed to -- the reader
may have closed the pane outright, quit the editor and left a bare
shell behind, or repurposed the pane for something else entirely.
Checking only the pane id would still find it in any of those cases
and reuse it anyway, typing `[editor] reuse`'s keystrokes into
whatever is actually running there instead of the editor.
`pane_alive` is the thin, untested half that asks tmux for both the
pane id and its current foreground command; `pane_is_listed` is the
pure half that decides, so the actual matching is exercised the same
way `resolve`/`tmux_argv` already are.

```rust name=pane_alive path=tui/editor.rs
...
`pane_current_command` reports a process by its basename regardless
of the full path `[editor] command`/`$EDITOR` used to spawn it, so
`pane_alive` needs to compare against that same basename, not
`argv[0]` verbatim.

```rust name=editor_name path=tui/editor.rs
...
`[editor] reuse` is keystrokes, not argv -- `cmd::substitute` fills in
`{file}`/`{line}`, but there is no `cmd::split` step, since the result
is typed into a running program rather than spawned as one.
`send_keys_argv` is what turns that substituted string into the
`tmux send-keys` call itself.

```rust name=send_keys_argv path=tui/editor.rs
